...
fn find_edge_collision(
    ...
    for (u, v) in bounding_box.grid_lines(u_axis, v_axis) {
        ...
        let edge_normal1 =
            (edge_normal1 - edge_normal0 * edge_normal0.mip(&edge_normal1)).lorentz_normalized();
...
fn find_vertex_collision(
    ...
    for (x, y, z) in bounding_box.grid_points(0, 1, 2) {
        ...
        let vertex_normal0 =
            MVector::new(1.0, 0.0, 0.0, layout.grid_to_dual(x)).lorentz_normalized();
...
mod tests {
    ...
    fn sanity_check_normal(ray: &Ray, hit: &ChunkCastHit) {
        ...
        let ray = math::translate(
            &ray.position,
            &ray.ray_point(hit.tanh_distance).lorentz_normalized(),
        ) * ray;

        // Project normal to be perpendicular to the ray's position
        let corrected_normal =
            (hit.normal + ray.position * hit.normal.mip(&ray.position)).lorentz_normalized();
