...
pub fn ensure_nearby(graph: &mut Graph, start: &Position, distance: f32) {
    ...
    let start_p = start.local * MUnitPointVector::origin();
    ...
    while let Some((node, current_transform)) = pending.pop_front() {
        for side in Side::iter() {
            ...
            let neighbor_p = neighbor_transform * MUnitPointVector::origin();
...
pub fn nearby_nodes(
    ...
    while let Some(current) = pending.pop_front() {
        let current_p = current.transform * MUnitPointVector::origin();
    ...
    pub fn new(graph: &'a Graph, position: Position, ray: &'a Ray, radius: f32) -> Self {
        ...
        for vertex in Vertex::iter() {
            let vertex_cosh_distance =
                (*vertex.node_to_dual() * position.local * MUnitPointVector::origin()).w;
